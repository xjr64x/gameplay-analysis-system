...
# Healthcheck: verify we can at least run the CLI
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD python main.py check --deps || exit 1

# Default: run batch analysis
# Override with: docker run ... python main.py analyze VIDEO --map MAP --mode MODE
# Or for interactive: docker run -it ... python main.py analyze VIDEO --map MAP --mode MODE
